...
execute 'Move mongodb archive to server' do
...
template '/apps/mongoapp/conf/mongod.conf' do
  ...
  group 'was'
  mode '0755'
  not_if { File.exist?('/apps/mongoapp/conf/mongod.conf') }
end
...
unless File.exist?("/apps/mongoapp/conf/initiate")
  include_recipe "::initiate"
end
